...
type CoffeeDrink struct {
	...
	Origin           *string          `json:"origin,omitempty" validate:"omitempty,max=32"`
	TemperatureStyle TemperatureStyle `json:"temperatureStyle" validate:"required"`
}

type CoffeeDrinks = []CoffeeDrink

type GetCoffeeDrinks struct {
	TemperatureStyle *TemperatureStyle `query:"temperatureStyle" validate:"omitempty,oneof=COLD HOT"`
}
